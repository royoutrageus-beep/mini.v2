...
# ════════════════════════════════════════════════════════
#  ⚙️  CONFIG — INPUT API KEY LO DI SINI BRO
# ════════════════════════════════════════════════════════
#
#  Step 1: Daftar di https://datasectors.com/register
#  Step 2: Login → API Keys → Create Key
#  Step 3: Paste key lo di bawah (format: ds_live_xxxxx)
#
# Semua key disimpan di Streamlit Secrets — tidak ada yang hardcode di sini!
...
# Session state init
if True:
    if "ds_calls_today" not in st.session_state: st.session_state.ds_calls_today = 0
    if "ds_date"        not in st.session_state: st.session_state.ds_date = None
    if "data_source"    not in st.session_state: st.session_state.data_source = "Belum scan"
    now_d = datetime.now(pytz.timezone("Asia/Jakarta")).date()
    if st.session_state.ds_date != now_d:
        st.session_state.ds_calls_today = 0
        st.session_state.ds_date = now_d
...
# ════════════════════════════════════════════════════
#  DATASECTORS HYBRID ENGINE
# ════════════════════════════════════════════════════

def ds_ok(n=1):
    return (bool(DATASECTORS_API_KEY)
            and (st.session_state.ds_calls_today + n) <= DS_DAILY_QUOTA)

def ds_bump(n=1):
    st.session_state.ds_calls_today += n
...
def fetch_intraday_ds(tickers_raw):
    all_dfs = {}
...
def fetch_intraday_yf(tickers, chunk=25):
    all_dfs = {}
...
def get_market_regime():
    try:
        ...
        if price < ema20:
            regime = "RED";      detail = f"IHSG {price:,.0f} < EMA20 → Bearish"
        elif price > ema20 and price > ema55:
            regime = "GREEN";    detail = f"IHSG {price:,.0f} > EMA20 & EMA55 → Bullish"
        else:
            regime = "SIDEWAYS"; detail = f"IHSG {price:,.0f} antara EMA20-EMA55 → Sideways"
...
def analyze_watchlist(tickers_raw, mode="Reversal 🎯"):
    ...
    for t in tickers_raw:
        ...
        if df is None or len(df) < 55:
            results.append({"Ticker":t,"Price":0,"Score":0,"Signal":"-",
                            "RSI-EMA":0,"Stoch K":0,"RVOL":0,"BB%":0,
                            "Trend":"-","TP":0,"SL":0,"R:R":0,
                            "ROC 3B%":0,"VWAP":0,"ATR":0,
        ...
        try:
            ...
            results.append({"Ticker":t,"Price":int(close),"Score":sc,"Signal":sig,
                ...
                "ATR":round(atr,0),"Reasons":" · ".join(reasons),"_class":get_card_class(sig)})
        except Exception as ex:
            results.append({"Ticker":t,"Price":0,"Score":0,"Signal":f"Error: {str(ex)[:30]}",
                            "RSI-EMA":0,"Stoch K":0,"RVOL":0,"BB%":0,
                            "Trend":"-","TP":0,"SL":0,"R:R":0,"ROC 3B%":0,
                            "VWAP":0,"ATR":0,"Reasons":"","_class":""})
    return results

def fetch_intraday(tickers_yf, chunk=25):
    """Hybrid: DataSectors → yFinance fallback."""
    tickers_raw = [t.replace(".JK","") for t in tickers_yf]
    if ds_ok(1):
        result = fetch_intraday_ds(tickers_raw)
        if result:
            st.session_state.data_source = "DataSectors 🟢"
            return result
        st.session_state.data_source = "yFinance (DS gagal) 🟡"
    else:
        remaining = DS_DAILY_QUOTA - st.session_state.ds_calls_today
...
<div style="display:flex;gap:10px;margin-bottom:10px;flex-wrap:wrap;">
  <div style="font-family:Space Mono,monospace;font-size:10px;padding:4px 12px;
       border-radius:20px;background:rgba(0,229,255,.08);border:1px solid rgba(0,229,255,.2);color:#00e5ff;">
    📡 {st.session_state.data_source}
  </div>
  <div style="font-family:Space Mono,monospace;font-size:10px;padding:4px 12px;
       border-radius:20px;background:rgba(0,0,0,.3);border:1px solid #1c2533;color:{qcol};">
...
<div style="background:rgba(0,0,0,.4);border:1px solid {rcolor}44;border-radius:8px;
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
    ...
    <div style="text-align:right;font-family:Space Mono,monospace;">
      ...
      <div style="font-size:9px;color:#4a5568;letter-spacing:1px;">
        EMA20 {ema20:,.0f} · EMA55 {ema55:,.0f}
      </div>
    </div>
  </div>
</div>
...
tab_scanner, tab_watchlist = st.tabs(["🔥 Scanner Intraday", "👁️ Watchlist Analyzer"])
    ...
    try:
        ...
        for i, ticker_yf in enumerate(tickers):
            ...
            try:
                ...
                atr=float(r['ATR'])
                slm=rcfg.get("sl_mult",0.8)  # regime-adjusted SL
        ...
        if not results:
            ...
            <div class="metric-row">
              <div class="metric-card" style="border-top-color:{rcfg['color']}"><div class="metric-label">Regime</div><div class="metric-value" style="font-size:16px;color:{rcfg['color']}">{regime}</div><div class="metric-sub">{ihsg_price:,.0f} {chg_sym}{abs(ihsg_chg):.2f}%</div></div>
...
with tab_watchlist:
    ...
    with wc2:
        wl_mode = st.radio("Mode Analisa",
            ["Scalping ⚡","Momentum 🚀","Reversal 🎯"], key="wl_mode")
        # Auto-suggest mode berdasarkan regime
    ...
    if wl_run and wl_input.strip():
        raw_wl = list(dict.fromkeys([
            t.strip().upper() for line in wl_input.split("\n")
        ...
        if not raw_wl:
            ...
            <div class="metric-row" style="margin-top:16px;">
              <div class="metric-card orange"><div class="metric-label">Dipantau</div>
                <div class="metric-value">{len(raw_wl)}</div></div>
              <div class="metric-card green"><div class="metric-label">GACOR 🔥</div>
                <div class="metric-value">{len(gacor_wl)}</div></div>
              <div class="metric-card amber"><div class="metric-label">POTENSIAL</div>
                <div class="metric-value">{len(pot_wl)}</div></div>
              <div class="metric-card"><div class="metric-label">Data OK</div>
                <div class="metric-value">{len(ok_res)}</div></div>
            </div>""", unsafe_allow_html=True)
            ...
            if not df_wl.empty:
                ...
                st.dataframe(df_wl[show], width='stretch', hide_index=True,
                    column_config={
                        "Score":   st.column_config.ProgressColumn("Score",min_value=0,max_value=6,format="%.1f"),
                        "RSI-EMA": st.column_config.NumberColumn("RSI-EMA",format="%.1f"),
    ...
    elif wl_share and "wl_results" in st.session_state:
        now_str = datetime.now(jakarta_tz).strftime("%d %b %Y %H:%M")
        txt  = f"🔥 THETA TURBO WATCHLIST\n⏰ {now_str} WIB\n"
        txt += f"📊 Mode: {st.session_state.get('wl_mode_used','')} | Regime: {regime}\n"
        txt += "─"*28 + "\n"
        ...
        <div style="text-align:center;padding:48px;color:#4a5568;font-family:Space Mono,monospace;">
          ...
          <div style="font-size:10px;margin-top:8px;color:#2d3748;">
            Bisa 1 atau banyak sekaligus · Pisah koma atau enter<br>
